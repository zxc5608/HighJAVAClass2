	  ...
	  $('.box').on('click', '.action', function(){
		  ...
		  if(vname == 'modify'){
			 ...
			 $('#uModal').modal('show');
			  ...
			  reply = pbody.find('.tspan').html(); 	 // 답변
			  ...
			  // uform에 출력
			  $('#uform #subject').val(cont);
			  //$('#uform #reply').val(title);
			  $('#uform #reply').val(reply);
...
// 글 수정창에서 글 수정후 확인 버튼 클릭
	$('#usend').on('click', function() {
		...
		board.content = $('#uform #subject').val();
		board.reply = $('#uform #reply').val();
		board.seq = vidx;
			      ...
			      <div class="modal-content">
			        ...
			        <form id="uform"> 
						...
						<label style="color: #08088A;">내용</label> <br> 
							<textarea rows="10" cols="50" name="content" id="subject" class="txt" style="color: black;"></textarea>
							<br>
							
						<label style="color: #08088A;">답변</label> <br> 
							<textarea rows="10" cols="50" name="reply" id="reply" class="txt" style="color: black;"></textarea>
							<br>
